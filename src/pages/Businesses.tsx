...
import { Plus, Building2, Trash2, Users, CheckCircle, RotateCcw } from "lucide-react";
...
export function Businesses() {
  ...
  const createBiz = trpc.businesses.create.useMutation({
    onSuccess: () => { setOpen(false); setForm({ name: "", slug: "", address: "", phone: "", email: "", plan: "basic", isMultiLocation: true }); utils.businesses.list.invalidate(); toast.success("Business created"); },
    ...
    <Layout>
      <div className="space-y-6">
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              <DialogContent className="bg-white">
                ...
                <div className="space-y-3">
                  ...
                  <Button
                    onClick={() => createBiz.mutate(form)}
                    disabled={!form.name || !form.slug || createBiz.isPending}
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {businesses?.map(b => (
            <Card key={b.id} className={`border-[#E8E0D8] ${b.id === user?.currentBusinessId ? "ring-2 ring-[#C73E1D]" : ""}`}>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  {b.id === user?.currentBusinessId && (
                    <span className="rounded-full bg-[#C73E1D]/10 px-2 py-0.5 text-xs text-[#C73E1D]">Active</span>
                  )}
                </div>
                <p className="text-xs text-[#8D8A87]">{b.slug} · {b.plan}</p>
              </CardHeader>
              <CardContent className="space-y-2">
                ...
                <div className="flex items-center gap-2 pt-2 flex-wrap">
                  ...
                  {b.id === user?.currentBusinessId && user?.role === "owner" && (
                    <Button
                      size="sm"
                      variant="outline"
