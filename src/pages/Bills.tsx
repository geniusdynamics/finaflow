...
export function Bills() {
  ...
  const [recForm, setRecForm] = useState<{ locationId: string; supplierId: string; categoryId: string; description: string; amount: string; frequency: "daily" | "weekly" | "monthly" | "quarterly" | "annually"; nextDueDate: string }>({
    locationId: "",
    supplierId: "",
    categoryId: "",
    description: "",
  ...
  useEffect(() => {
    ...
    if (!bill) return;
    const matches = getFundingAccounts(payForm.paymentMethod, accounts as FundingAccount[] | undefined, bill.locationId);
    if (matches.length === 1) {
                  ...
                  <div className="flex items-center justify-end gap-1">
                    ...
                    {canPay && bill.status !== "paid" && (
                      <Dialog open={paymentOpen === bill.id} onOpenChange={v => { setPaymentOpen(v ? bill.id : null); setPaymentError(null); }}>
                        <DialogTrigger asChild><Button size="sm" variant="outline" className="border-[#C73E1D] text-[#C73E1D]" onClick={() => { setPayForm({ paymentMethod: "wallet", amount: "", paymentDate: getLocalDateString(), reference: "", accountId: "" }); setPaymentError(null); }}>Pay</Button></DialogTrigger>
