              ...
              <Dialog open={empOpen} onOpenChange={setEmpOpen}>
                ...
                <DialogContent className="bg-white max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle className="font-serif text-xl">Add Employee</DialogTitle></DialogHeader>
                  <form onSubmit={handleEmp} className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3"><div><Label>Full Name</Label><Input value={empForm.fullName} onChange={e => setEmpForm(p => ({ ...p, fullName: e.target.value }))} required /></div><div><Label>Phone</Label><PhoneInput value={empForm.phone} onChange={value => setEmpForm(p => ({ ...p, phone: value }))} /></div></div>
                        ...
                        <Dialog open={empEditOpen === emp.id} onOpenChange={v => setEmpEditOpen(v ? emp.id : null)}>
                          ...
                          <DialogContent className="bg-white max-h-[90vh] overflow-y-auto">
                            ...
                            <form onSubmit={e => { e.preventDefault(); const userIdNum = editEmpForm.userId ? Number(editEmpForm.userId) : null; updateEmployee.mutate({ id: emp.id, ...editEmpForm, userId: userIdNum }); }} className="space-y-3">
                              <div className="grid grid-cols-2 gap-3"><div><Label>Full Name</Label><Input value={editEmpForm.fullName} onChange={e => setEditEmpForm(p => ({ ...p, fullName: e.target.value }))} required /></div><div><Label>Phone</Label><PhoneInput value={editEmpForm.phone} onChange={value => setEditEmpForm(p => ({ ...p, phone: value }))} /></div></div>
